...
public class DisplayServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try(Connection connection=DBconn.initializeDatabase()) {
			...
			String sql="SELECT `cust_number`,`name_customer`,`invoice_id`,`total_open_amount`,`due_in_date`,`clear_date`,`notes` FROM mytable LIMIT ?,?";
			
			PreparedStatement stmt=connection.prepareStatement(sql);
			stmt.setInt(1,pageNum*count);
			stmt.setInt(2,count);
			...
			System.out.println("ClassIssue");
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			System.out.println("SqlIssue");
		}
